...
> 关于"直接导入"、"项目旁复制"和"指定文件夹"三种模式的详细区别、风险及适用场景，请参阅 **[《5-高级设置与预设管理》](./5-advanced-settings-preset-management.md)**。

### 导入行为（决定如何放入时间轴）

这组选项控制素材导入AE后，是否以及如何被添加到当前合成中。插件提供了4个独立的导入行为选项，您可以直接选择其中任意一个：

- **不导入合成**: 素材仅复制到项目文件夹，不导入到合成，供您后续手动使用。
- **创建预合成**: 素材创建预合成并放置在当前时间位置，便于您为该素材添加独立的效果或动画。
- **当前时间**: 素材导入到合成并放置在当前时间指针位置。
- **时间轴开始**: 素材导入到合成并移至时间轴开始处（0秒位置）。

> 关于这些模式的详细功能说明，请参阅 **[《5-高级设置与预设管理》](./5-advanced-settings-preset-management.md)**。
...
现在您已经熟悉了主界面和核心设置，可以继续阅读 **[《3-核心工作流：处理导入的各类素材》](./3-handling-imported-assets.md)**，深入了解不同素材在AE中的具体处理方式。