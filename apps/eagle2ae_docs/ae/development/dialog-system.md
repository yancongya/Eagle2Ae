...
3.  **虚拟对话框系统**: 用于演示模式，完全由JavaScript模拟的对话框系统。

在多面板支持架构下，对话框系统能够为每个面板实例提供独立的对话框服务，确保各面板的用户交互互不干扰。
...
### 2.1. 系统概述
演示模式下的虚拟对话框系统是一个完全由JavaScript实现的对话框模拟器，用于在Web环境中展示对话框效果，无需安装After Effects。

### 2.2. 架构与实现
- **核心文件**: `Eagle2Ae-Ae/js/demo/demo-dialog.js`
- **实现原理**: 使用纯JavaScript和CSS创建模拟的对话框界面，完全兼容CEP环境中的对话框样式和交互。
- **调用流程**:
    1.  `demo-mode.js`检测到演示模式环境
    2.  自动切换对话框调用到虚拟对话框系统
    3.  创建虚拟对话框并显示模拟数据
