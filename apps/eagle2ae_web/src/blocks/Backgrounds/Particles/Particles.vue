...
watch([() => props.mouseX, () => props.mouseY], ([newX, newY]) => {
  if (props.moveParticlesOnHover) {
    mouseRef.value = { x: newX, y: newY };
  }
});
...
const initParticles = () => {
  ...
  if (props.moveParticlesOnHover) {
    // container.addEventListener('mousemove', handleMouseMove); // This is now handled by props
  }
