...
- **拖拽悬浮选择**：拖拽文件时悬浮到按钮即可选择导入行为，提升操作效率
- **CEP环境兼容**：完美兼容After Effects CEP环境，支持Web和CEP双环境
- **拟态滚动条**：精致的拟态风格滚动条，支持深色和浅色主题
- **独显模式优化**：优化的独显模式布局，最大化内容显示区域
...
### v2.5.0 拖拽悬浮选择增强版 + UI优化
- **拖拽悬浮选择导入行为**：拖拽文件时悬浮到按钮即可动态切换导入方式
- **移除拖拽弹窗提示**：简化交互体验，提供更清洁的界面
- **实时视觉反馈**：使用现有高亮样式，提供清晰的选中状态指示
- **独显模式布局优化**：消除不必要的垂直间距，最大化内容显示区域
- **拟态滚动条样式**：精致的拟态风格滚动条，支持深色和浅色主题
- **CEP环境兼容性修复**：完美兼容After Effects CEP环境，解决布局异常问题
- **跨环境一致性**：确保Web和CEP环境下的UI布局和样式一致
