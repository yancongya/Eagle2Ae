...
class SettingsManager {
    ...
    validateExportSettings(exportSettings) {
        ...
        // 验证导出模式
        if (!exportSettings.mode || !Object.values(this.ExportModes).includes(exportSettings.mode)) {
            errors.push('无效的导出模式');
        }

        // 验证自定义路径（当模式为自定义文件夹时）
        if (exportSettings.mode === this.ExportModes.CUSTOM_FOLDER) {
            if (!exportSettings.customExportPath || exportSettings.customExportPath.trim() === '') {
                errors.push('自定义导出路径不能为空');
            }
        }

        // 验证项目旁文件夹名称
        if (exportSettings.mode === this.ExportModes.PROJECT_ADJACENT) {
            if (!exportSettings.projectAdjacentFolder || exportSettings.projectAdjacentFolder.trim() === '') {
                errors.push('项目旁文件夹名称不能为空');
            }
        }

        if (errors.length > 0) {
            throw new Error(`导出设置验证失败: ${errors.join(', ')}`);
        }
    ...
    saveExportSettings(exportSettings) {
        try {
            ...
            // 更新导出设置
            this.settings.exportSettings = { ...exportSettings };
            ...
            // 触发变更事件
            this.notifyListeners('exportSettings', this.settings.exportSettings);
