...
// Eagle剪切板监控器类定义（内联）
class EagleClipboardMonitor {
    constructor(eagle2ae) {
        this.eagle2ae = eagle2ae;
        this.isMonitoring = false;
        this.monitorInterval = null;
        this.lastHasFiles = false;
        this.pollInterval = 500; // 500ms检测间隔
        this.cooldownTime = 1000; // 1秒冷却期，避免重复提醒
        this.lastNotificationTime = 0;

        // 支持的文件格式列表 - 添加更多可能的格式
        this.fileFormats = [
            'files',                    // 通用文件格式
            'public.file-url',          // 文件URL格式
            'application/x-file-list',  // 文件列表格式
            'text/uri-list',           // URI列表格式
            'public.file-list',        // 文件列表格式
            'CF_HDROP',                // Windows特有格式
            'NSFilenamesPboardType',   // macOS特有格式
            'text/plain',              // 临时添加文本格式用于测试
            'application/x-qt-windows-mime;value="FileNameW"', // Qt格式
            'application/x-qt-windows-mime;value="FileName"'   // Qt格式
        ];

        console.log('🎯 剪切板监控器已初始化');
    }

    startMonitoring() {
        if (this.isMonitoring) {
            console.warn('剪切板监控已在运行中');
            return;
        }
        ...
        console.log('🎯 剪切板文件监控已启动');
        this.eagle2ae.log('剪切板文件监控已启动', 'success');

        // 显示启动通知
        this.showNotification('剪切板监控已启动', '开始监控文件复制操作', 2000);
    }

    async checkForFileChanges() {
        try {
            const hasFiles = this.detectFiles();
            const now = Date.now();

            // 调试日志 - 显示检测状态
            if (hasFiles !== this.lastHasFiles) {
                console.log(`🔍 剪切板状态变化: ${this.lastHasFiles} -> ${hasFiles}`);
            }

            // 检测到文件复制且满足条件时提醒
            if (hasFiles &&
                !this.lastHasFiles &&
                (now - this.lastNotificationTime) > this.cooldownTime) {

                console.log('🎯 触发文件检测处理');
                await this.handleFileDetected();
                this.lastNotificationTime = now;
            }

            this.lastHasFiles = hasFiles;

        } catch (error) {
            console.error('剪切板文件检测失败:', error);
        }
    }
    ...
    detectFiles() {
        ...
        if (this.debugCounter % 20 === 0) {
            console.log(`🔍 剪切板检测运行中... (第${this.debugCounter}次)`);
        }
        ...
        for (const format of this.fileFormats) {
            try {
                ...
                // 记录不支持的格式（仅首次）
                if (!this.loggedUnsupportedFormats) {
                    this.loggedUnsupportedFormats = new Set();
                }
                if (!this.loggedUnsupportedFormats.has(format)) {
                    console.warn(`⚠️ 格式 ${format} 不支持: ${error.message}`);
                    this.loggedUnsupportedFormats.add(format);
                }
                continue;
            }
        }
    ...
    async handleFileDetected() {
        console.log('📁 检测到文件复制操作！');

        try {
            // 显示文件复制通知
            this.showNotification(
                'Eagle2Ae - 文件复制检测',
                '检测到文件复制操作！可以导入到AE中使用',
                4000
            );

            // 记录到Eagle2Ae日志
            this.eagle2ae.log('📋 检测到文件复制操作', 'info');

            // 发送消息到AE扩展（如果连接）
            this.sendToAE();

        } catch (error) {
            console.error('处理文件检测失败:', error);
        }
    }

    showNotification(title, body, duration = 3000) {
        if (!eagle.notification) {
            console.warn('Eagle通知API不可用');
            return;
        }
    ...
    sendToAE() {
        const message = {
            type: 'clipboard_files_detected',
            timestamp: Date.now(),
            source: 'eagle_clipboard_monitor',
            data: {
                detected: true,
                message: '检测到文件复制操作'
            }
        ...
        // 添加到消息队列
        if (this.eagle2ae.messageQueue) {
            this.eagle2ae.messageQueue.push(message);
        }

        // 添加到Eagle日志
        if (this.eagle2ae.eagleLogs) {
            this.eagle2ae.eagleLogs.push({
    ...
    stopMonitoring() {
        ...
        console.log('⏹️ 剪切板文件监控已停止');
        this.eagle2ae.log('剪切板文件监控已停止', 'info');

        // 显示停止通知
        this.showNotification('剪切板监控已停止', '文件复制监控已关闭', 2000);
    }
    ...
    getStatus() {
        return {
            ...
            lastHasFiles: this.lastHasFiles,
            supportedFormats: this.fileFormats.length,
...
class Eagle2Ae {
    ...
    startClipboardMonitoring() {
        try {
            console.log('🔍 开始启动剪切板监控...');

            // 检查Eagle剪切板API是否可用
            if (typeof eagle === 'undefined') {
                this.log('⚠️ Eagle对象不可用，跳过剪切板监控', 'warn');
                return;
            }

            if (!eagle.clipboard) {
                this.log('⚠️ Eagle剪切板API不可用，跳过剪切板监控', 'warn');
                return;
            }

            console.log('✅ Eagle API检查通过，创建监控器实例...');

            // 创建剪切板监控器实例（类已内联，无需检查）
            this.clipboardMonitor = new EagleClipboardMonitor(this);
            this.clipboardMonitor.startMonitoring();

            this.log('🎯 剪切板文件监控已启动', 'success');
            console.log('✅ 剪切板监控启动成功');

        } catch (error) {
            this.log(`剪切板监控启动失败: ${error.message}`, 'error');
            console.error('剪切板监控启动失败:', error);
            console.error('错误堆栈:', error.stack);
        }
    }
...
eagle.onPluginBeforeExit((event) => {
    ...
    if (eagle2ae) {
        ...
        // 清理剪切板监控
        if (eagle2ae.clipboardMonitor) {
            eagle2ae.stopClipboardMonitoring();
        }
